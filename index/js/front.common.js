        ...
        _keyup: function(e) {
            ...
            if (e.keyCode == 38 || e.keyCode == 40) {
                ...
                if (that.searchRecommend.is(':visible')) {
                    that.recommend = that.searchRecommend.find('.active').hasClass('ol') ? that.searchRecommend.find('.popular') : that.searchRecommend.find('.recent');
                    switch (e.keyCode) {
                        case 38:
                            that.keyIndex--;
        ...
        _setPoint: function(i, k) {
            ...
            if (!that._isMinimal) {
                ...
                that._pointArr[k].data({ 'i': i, 'k': k, 'cx': parseFloat(that._dataPathArr[i][k * 2]) + 0.5, 'isTop': isTop }).mouseover(function() {
                    ...
                    var rank = parseFloat(that._dataPathArr[i][cy_idx]),
                        rank_idx = 0;
